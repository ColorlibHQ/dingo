 ...
 */
	...
	final class Dingo{
		...
		private function setup(){
			...
			$enqueu = new dingo_Enqueue();
			$enqueu->scripts = $this->enqueue() ;
			$enqueu->dingo_scripts_enqueue_init() ;

		}
		// Theme Support
		public function support(){
			// content width
	        $GLOBALS['content_width'] = apply_filters( 'dingo_content_width', 751 );

	        
	        // text domain for translation.
	        load_theme_textdomain( 'dingo', DINGO_DIR_PATH . '/languages' );
			...
			add_theme_support( 'post-thumbnails', array( 'post', 'page', 'food' ) );
			
			// Site logo size
			add_image_size( 'site_logo_153x63', 153, 63, true );
					
			// About section image size
			add_image_size( 'dingo_banner_section_846x1046', 846, 1046, true );
			
			// Testimonial client image size
			add_image_size( 'review_img_150x150', 150, 150, true );

			// Latest post thumbnail Widget thumbnail size
			add_image_size( 'dingo_widget_post_thumb', 80, 80, true );

			// Exclusive services thumbnail size
			add_image_size( 'dingo_exclusive_serv_360x330', 360, 330, true );

			// About section image size
			add_image_size( 'dingo_about_section_612x612', 612, 612, true );

			// Food menu thumb size
			add_image_size( 'dingo_food_menu_thumb_165x163', 165, 163, true );

			// Testimonial thumb size
			add_image_size( 'dingo_testimonial_thumb_140x140', 140, 140, true );

			// Home blog post image size
			add_image_size( 'dingo_latest_blog_370x345', 370, 345, true );

			// Single blog post image size
			add_image_size( 'dingo_single_blog_750x375', 750, 375, true );
			add_image_size( 'dingo_np_thumb', 60, 60, true );
	        ...
	        register_nav_menus( array(
	            'primary-menu'   => esc_html__( 'Primary Menu', 'dingo' ),
				'social-menu'    => esc_html__( 'Social Menu', 'dingo' ),
				'important-links'=> esc_html__( 'Important Link', 'dingo' )
	        ) );
		...
		private function enqueue(){
			...
			$cssPath = DINGO_DIR_CSS_URI;
			$jsPath  = DINGO_DIR_JS_URI;
		...
		private function google_font(){
			...
			if ( 'off' !== _x( 'on', 'Google font: on or off', 'dingo' ) ) {
		...
		private function customizer_init(){
			...
			// Instantiate dingo theme customizer
			$dingo_theme_customizer = new dingo_theme_customizer();
		}
		...
		public function epsilon_quickie(){
				...
				'links' => array(
					array(
						'link_to'   => 'dingo_theme_options_panel',
