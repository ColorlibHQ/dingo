...
// social media
if ( ! function_exists( 'dingo_social' ) ) {
	function dingo_social( $args = array()  ){
		...
		$url = dingo_opt('dingo_social_url');
...
function dingo_contact_form7_shortcode(){

    // contact form list
    $getforms['cs'] = __( 'Custom Shortcode', 'dingo' );
    // Instruction
    $Instruction = ''; 
    ...
    if( defined('WPCF7_VERSION') ){
        ...
        if( $loop->have_posts() ){
            ...
            $Instruction = __( 'Contact form not found.', 'dingo' );
        }
    }else{
        $url = admin_url( 'plugins.php' );
        
        $Instruction = sprintf( __( 'If you want to use contact form 7, Please install and active contact form 7 plugin. %s Click here to install %s  ' , 'dingo' ), '<a target="_blank" href="'.esc_url( $url ).'">', '</a>'  );
    }
...
// Popular post count
function dingo_set_post_views($postID) {
    $count_key = 'dingo_post_views_count';
...
// blog post categoty 
function dingo_get_post_cat(){
    $cats = get_categories();

    $categories = array( 'na' => esc_html__( 'Select post category', 'dingo' ) );
